...
class VisualAnalysis(QWidget):
    ...
    def __init__(self):
        super().__init__()
        # 원도우 창 정보 정의
        self.analysis_screen_resolution_multiples = 2
        self.analysis_screen_width = 16 * 10
        self.analysis_screen_height = 16 * 10
        # 원도우 창 설정
    ...
    def paintEvent(self, event):
        ...
        graphic = QPainter()
        graphic.begin(self)
        graphic.setPen(QPen(QColor.fromRgb(255, 255, 255), 0.1, Qt.SolidLine))
        graphic.setBrush(QBrush(Qt.blue))
        # 직사각형 그리기
